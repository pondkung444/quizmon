...
import PlanProgressCard, { type PlanProgress, type PlanRow } from "./PlanProgressCard";
import {
  accuracyBgClass,
  accuracyTextClass,
  subjectLabel,
...
// เนื้อหาหน้าภาพรวม ใช้ร่วมกันระหว่าง /guardian/[studentId] (viewerMode="guardian" default — ผลลัพธ์เหมือนเดิมทุกไบต์)
// กับ /my-plan (viewerMode="self" — นักเรียน self-serve ดูของตัวเอง) ผู้เรียกเช็ค access แล้ว: ที่นี่ไม่เช็คซ้ำ
export default async function StudentOverview({
  ...
  // viewerMode="self": ข้าม — RPC นี้เขียน audit log ของ "ผู้ปกครองดู" (is_guardian_admin) นักเรียนดูของตัวเองไม่ต้อง log
  if (viewerMode !== "self") {
    ...
    <div className="flex flex-col gap-4">
      ...
      <Card className="flex items-center gap-4">
        ...
        <div className="min-w-0">
          <p className="truncate text-lg font-bold text-gold-hi">{studentUsername}</p>
          {qmon ? (
      ...
      <PlanProgressCard studentId={studentId} plan={plan} progressByKey={progressByKey} viewerMode={viewerMode} />
