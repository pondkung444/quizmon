...
import { BOSSES, CARDS, INTENTS, MAX_ENERGY, MAX_TURNS, checkPreview, incomingDamage, phaseNumber, damageProgress, growthAdvice, type Battle, type CardId } from "@/lib/raid/cards/engine";
...
export default function CardBattleArena({ battle: b, petName, petImage, bestProgress, busy, error, animateTurn, onPlay, onReload, onExit, onReward, rewardReady, demo,question,feedback,onAnswer,onContinue }: ArenaProps) {
  ...
  const card = selection ? CARDS[selection] : null;
    ...
    <main className={styles.shell} style={{ "--battle-accent": boss.accent } as CSSProperties}>
      ...
      <div className={styles.layout}>
        <header className={styles.header}>
          ...
          <div className={styles.round}><span>เทิร์น</span><strong>{b.turn}<small> / {MAX_TURNS}</small></strong></div>
        </header>
        ...
        <section className={styles.stage} aria-label="สนามต่อสู้">
          ...
          <div key={`boss-${animateTurn}`} className={`${styles.bossSprite} ${animateTurn && last?.dealt ? styles.bossHit : ""} ${b.outcome === "win" ? styles.vanquished : ""}`}>
            ...
            {animateTurn > 0 && last && last.dealt > 0 && <span className={styles.damageFloat}>−{last.dealt}</span>}
          </div>
          ...
          {!b.outcome && <div className={styles.intent} key={`${b.turn}-intent`}>
            ...
            <div><span>บอสเตรียมใช้</span><h2>{intent.name}</h2><p>{intent.hint}</p></div>
            <strong className={styles.threat}>{incomingDamage(b) > 0 ? `~${incomingDamage(b)}` : "—"}<small>ดาเมจก่อนรับมือ</small></strong>
          </div>}
        </section>
        ...
        <section className={styles.controls} aria-label={b.outcome ? "ผลการต่อสู้" : "เลือกการ์ดรับมือ"}>
          ...
          <p className={styles.requirements}>เกณฑ์ชนะ: stat {statPercent(b.stats).toFixed(1)} / {STAT_REQUIREMENTS[b.bossId]}% · ตอบถูก {b.log.filter(e=>e.answerCorrect!==false).length}/{b.log.length} (ต้อง ≥60%)</p>
          {(question || feedback) ? <RaidLearningPanel key={`${(question || feedback!.question).revision}-${!!feedback}`} question={question || feedback!.question} feedback={feedback} busy={busy} onAnswer={onAnswer!} onContinue={onContinue!}/> : b.outcome ? (
            <div className={styles.summary}>
              <span className={styles.eyebrow}>{b.outcome === "win" ? "พิชิตผู้พิทักษ์" : "แผนรอบหน้าเริ่มที่นี่"}</span>
              <h2>{b.outcome === "win" ? "ทำได้แล้ว!" : b.defeatReason === "stats" || b.defeatReason === "learning" ? "ยังไม่ผ่านเกณฑ์ด่าน" : b.turn >= MAX_TURNS && b.hp > 0 ? "ครบเวลาท้าทายแล้ว" : "กลับมาตั้งหลักกัน"}</h2>
              <p>{b.outcome === "win" ? `${petName} พิชิต${boss.name} ใน ${b.turn} เทิร์น` : `ลดเลือดบอสได้ ${damageProgress(b)}% • สถิติเดิม ${bestProgress}%`}</p>
            ...
            <>
              <div className={styles.handHeader}>
                <div><span className={styles.eyebrow}>อ่านสถานการณ์ · เลือกจังหวะของเรา</span><h2>จะรับมืออย่างไร?</h2></div>
                <div className={styles.energy} aria-label={`พลัง ${b.energy} จาก ${MAX_ENERGY}`}><Zap size={15} />{Array.from({ length: MAX_ENERGY }, (_, i) => <i key={i} className={i < b.energy ? styles.energyFull : ""} />)}<span>{b.energy}/{MAX_ENERGY}</span></div>
              </div>
              ...
              <div className={styles.hand}>
                {b.hand.map((id) => {
                  const c = CARDS[id]; const Icon = ICONS[c.kind]; const affordable = c.cost <= b.energy;
                  return <button key={id} data-card={id} className={`${styles.card} ${styles[c.kind]} ${selection === id ? styles.selected : ""}`} aria-pressed={selection === id} onClick={() => choose(id)} disabled={busy}>
                    <div className={styles.cardTop}><span>{c.stat?.toUpperCase()}</span><span className={!affordable ? styles.lowEnergy : ""}><Zap size={12} />{c.cost}</span></div>
              ...
              <div className={styles.cardDetail} aria-live="polite">
                {card && preview ? <><div><strong>{card.name}</strong><span>{card.stat?.toUpperCase()} {preview.value} {selection !== "guard" && selection !== "strike" ? `/ เกณฑ์ ${preview.dc}` : ""}</span></div><p>{card.description}</p><small>{chanceText(selection!)} • {card.success}</small></> :
                  <p className={styles.prompt}>{busy ? "กำลังลงมือและบันทึกผล..." : "แตะการ์ดเพื่อดูผลก่อนใช้ • ใบที่ยังไม่ใช้เก็บไว้รอบหน้าได้"}</p>}
              </div>
              <button className={styles.primary} data-testid="play-card" disabled={busy || !selection || !preview?.affordable} onClick={play}>
                {busy ? "กำลังเตรียมโจทย์..." : !selection ? "เลือกการ์ดของเธอ" : !preview?.affordable ? "พลังไม่พอ • ลองตั้งหลัก" : `ตอบเพื่อใช้${CARDS[selection].name}`}<ArrowUpRight size={18} />
              </button>
          ...
          <details className={styles.journal}><summary>บันทึกการต่อสู้ · {b.log.length} เทิร์น</summary>
            {b.log.length === 0 ? <p>ยังไม่มีเทิร์นที่เล่น</p> : b.log.toReversed().map((entry) => <p key={entry.turn}><strong>{entry.turn}. {CARDS[entry.card].name}</strong> → {INTENTS[entry.intent].name}<br />{entry.note}{entry.roll !== null ? ` (ทอย ${entry.roll} / โอกาส ${entry.chance}%)` : ""}<br />ทำดาเมจ {entry.dealt} · รับ {entry.taken} · ฟื้น {entry.healed}</p>)}
          </details>
          <details className={styles.journal}><summary>วิธีเล่นและสเตตัสของมอน</summary><p>ลดเลือดบอสให้หมดภายใน 20 เทิร์น เลือกคำสั่งพื้นฐานได้เสมอ จบเทิร์นฟื้นพลัง 1 ตั้งแต่เทิร์น 13 บอสโจมตีแรงขึ้น การ์ดที่ใช้จะพักในกองทิ้งก่อนกลับมาจั่วได้ ไม่มี EXP จากการใช้การ์ด</p><p>{Object.entries(b.stats).map(([key, value]) => `${key.toUpperCase()} ${value}`).join(" · ")}</p></details>
        </section>
        <footer className={styles.footer}><Wind size={12} />{boss.subtitle}</footer>
      </div>
    </main>
  );
}
