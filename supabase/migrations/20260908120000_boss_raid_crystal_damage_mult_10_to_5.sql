-- Boss Raid balance retune (2026-09-08): crystal_damage multiplier rank*10 -> rank*5.
-- Data-driven: 4 live rooms tested 2026-09-08 showed only ~25% win rate vs 50-65% target.
-- Monte Carlo on real avg room stats: mult=10 needs ~80%+ room accuracy to win;
-- mult=5 moves that threshold to ~60-65% accuracy, matching the target win-rate band.
-- target_wrong (275) and target_correct (200) in start_boss_raid_game are unchanged --
-- the multiplier was the most sensitive lever. Only ONE line changes vs the
-- 20260907061810 definition (v_crystal_damage := v_new_rank * 5).
