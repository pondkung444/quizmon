-- Boss Raid balance fix (2026-09-05): crystal_damage double-dipped avg_def
-- (already baked into crystal_hp_max) making rooms nearly unloseable
-- (up to ~92 wrong answers/player to break the crystal). Fix requires two
-- coordinated changes or rooms become "always lose" instead:
--   1) crystal_damage per hit = flat rank*6 (tier-based), no longer /avg_def
--   2) boss target_correct halved 250->125 to rebalance win rate back to
--      the original 50-65% target now that the crystal breaks faster
-- Verified via Python simulator using the real DB formulas.
-- Confirmed live: room 2UJW49, N=15 (2026-09-05).
...
begin
  ...
  if v_is_correct and v_damage > 0 then
    ...
    -- balance fix 2026-09-05: flat rank*6, no longer /avg_def (was double-dipping
    -- avg_def that crystal_hp_max already multiplies in)
    v_crystal_damage := v_new_rank * 6;
...
declare
  ...
  c_target_correct constant numeric := 125;  -- เดิม 250 -> ลดครึ่งหนึ่ง (balance fix 2026-09-05: คู่กับ crystal_damage flat rank*6)
