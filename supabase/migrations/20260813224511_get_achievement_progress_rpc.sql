...
begin
  ...
  select 'training', (select count(*)::int from public.quiz_attempts where user_id = p_user_id)
  union all
  select 'daily_days', (select count(*)::int from completed_days)
  union all
  select 'daily_streak', (select coalesce((select len from current_streak), 0))
  union all
