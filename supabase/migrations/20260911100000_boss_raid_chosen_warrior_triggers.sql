-- Item 3: chosen_warrior fires ~once per session (heavy-tier entry only). Add:
--   - medium-tier entry trigger (mirrors the heavy check) — ~2x tier-based triggers/game
--   - a positive trigger tied to the room doing well: a class-wide correct streak
--
-- The positive trigger uses a DEDICATED counter (cw_streak_current) rather than
-- correct_streak_current: the trigger block is gated on `v_event_free`, so an exact-
-- equality check on the shared counter would be silently skipped whenever a random
-- weak_point/meteor happens to be active on the streak's 5th answer. A dedicated
-- counter with `>= N` + reset-on-fire waits for a free event slot and stays self-
-- limiting, while correct_streak_current keeps climbing to 8 for combo_burst untouched.
--
-- cw_streak_current lifecycle = correct_streak_current's reset points (wrong answer,
-- combo_burst) PLUS reset when the positive trigger fires. combo_burst's damage and
-- trigger threshold are unchanged; its UPDATE just also zeroes the new counter.
--
-- create or replace only otherwise. Base = live body (md5 5645a07eea75636932e9a25964b93850).

alter table public.boss_raid_sessions
  add column if not exists cw_streak_current int not null default 0;
comment on column public.boss_raid_sessions.cw_streak_current is
  'นับตอบถูกติดต่อกันทั้งห้อง สำหรับ trigger บวกของ chosen_warrior — reset เมื่อตอบผิด / combo_burst / chosen_warrior บวกยิง. แยกจาก correct_streak_current (ที่ combo_burst ใช้).';
