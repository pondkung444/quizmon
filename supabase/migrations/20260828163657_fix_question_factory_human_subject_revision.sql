...
create or replace function public.question_factory_record_human_review(
  p_run_key uuid,
  p_slot_key text,
...
begin
  ...
  if v_slot.author_revision + 1 <> p_subject_revision then
...
comment on function public.question_factory_record_human_review(
  uuid, text, bigint, integer, text, integer, text, text, text, jsonb, jsonb, text, text
) is 'Records a service-only human decision against candidate revision author_revision + 1, mapping candidate checksum, and latest QC-passed asset identity.';
