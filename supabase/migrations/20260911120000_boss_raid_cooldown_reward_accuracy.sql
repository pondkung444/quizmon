-- Item 4: per-player wrong-answer cooldown (4a) + reward accuracy threshold (4b).
--
-- 4a  3 consecutive non-correct (wrong OR timeout) per player -> 30s cooldown.
--     new cols boss_raid_participants.wrong_streak_current + cooldown_until.
--     submit_boss_raid_answer: gate blocked submits (never records) + set the
--     cooldown on the 3rd; get_next_boss_raid_question: return {cooldown:true}
--     while active, and on expiry clear it + reset the streak + force a fresh
--     question. Scoped to the normal question flow (meteor / chosen_warrior are
--     separate rare RPCs, not the spam vector).
-- 4b  reward eligibility requires >= 40% accuracy (correct/total answers) for
--     that player. distribute_boss_raid_rewards filters BEFORE ranking; rank
--     order stays total_damage desc. No change to the live TV leaderboard.
--
-- create or replace on the 3 functions (submit base = Item 3 body). No drops.

alter table public.boss_raid_participants
  add column if not exists wrong_streak_current int not null default 0,
  add column if not exists cooldown_until timestamptz;
comment on column public.boss_raid_participants.wrong_streak_current is
  'ตอบผิด/หมดเวลาติดกันของผู้เล่นคนนี้ — reset เมื่อตอบถูก และเมื่อคูลดาวน์หมด (Item 4a).';
comment on column public.boss_raid_participants.cooldown_until is
  'ห้ามส่งคำตอบจนถึงเวลานี้ (null = ไม่ติดคูลดาวน์). ตั้งเมื่อ wrong_streak_current ถึง 3.';
...
declare
  ...
  c_cw_correct_streak_n constant int := 5;
...
begin
  ...
  -- freeze: chosen_warrior (ไม่มีอายุ) หรือ meteor (ยังไม่หมดอายุ)
  ...
  -- Item 4a: กำลังคูลดาวน์ -> ไม่รับคำตอบ (auto-submit ค้างจากตอน freeze ก็ตกที่นี่)
  if v_p.cooldown_until is not null and v_p.cooldown_until > now() then
    return jsonb_build_object('cooldown', true, 'cooldown_until', v_p.cooldown_until,
      'is_correct', null, 'is_crit', false, 'damage_dealt', 0,
      'boss_hp', v_s.boss_hp, 'crystal_hp', v_s.crystal_hp,
      'current_tier', v_s.current_tier, 'crystal_damage', null,
      'status', v_s.status, 'result', v_s.result);
  end if;
  ...
  -- Item 4a: per-player wrong streak + คูลดาวน์ (3 ผิดติดกัน incl. timeout -> พัก 30 วิ)
  update public.boss_raid_participants
    set current_question_id = null, question_started_at = null,
        ...
        wrong_streak_current = case when v_is_correct then 0 else wrong_streak_current + 1 end,
        cooldown_until = case
          when not v_is_correct and wrong_streak_current + 1 >= 3
            then now() + interval '30 seconds'
          else null   -- ถ้าถึงตรงนี้แปลว่าคูลดาวน์เดิม (ถ้ามี) หมดอายุแล้ว — เคลียร์ทิ้ง
        end
  ...
  if v_is_correct then
    update public.boss_raid_sessions
      set wrong_streak_current = 0,
          ...
          cw_streak_current = cw_streak_current + 1
      where id = v_p.session_id
      returning correct_streak_current, cw_streak_current into v_combo_streak, v_cw_streak;
    v_wrong_streak := 0;
  ...
  -- chosen_warrior triggers — Item 3 (2026-09-11):
  --   negative: entering medium OR heavy tier for the first time, or a room-wide
  --     wrong streak reaching the tier threshold
  --   positive: room-wide correct streak >= c_cw_correct_streak_n (dedicated counter,
  --     reset on fire below -> self-limiting + waits out a busy event slot)
  ...
  then
    if not v_is_correct then
      ...
      if (v_new_tier = 'heavy'  and v_prev_tier <> 'heavy')
         or (v_new_tier = 'medium' and v_prev_tier = 'light')
         or v_wrong_streak >= v_streak_n
      then
        v_cw_trigger := true;
      end if;
    elsif v_cw_streak >= c_cw_correct_streak_n then
      v_cw_trigger := true;
    end if;
  end if;
  ...
  if v_cw_trigger then
    update public.boss_raid_sessions
      set wrong_streak_current = 0, cw_streak_current = 0
      where id = v_p.session_id;
    ...
    with w as (
      ...
      where p.session_id = v_p.session_id
        -- Item 4a: อย่าเลือกคนที่กำลังพักคูลดาวน์เป็นนักรบ (จะตอบไม่ได้ -> ห้อง deadlock)
...
begin
  ...
  -- Item 4a: คูลดาวน์รายคน — ระหว่างพักคืน {cooldown:true}; พอหมดเวลาให้เคลียร์
  -- + reset streak + บังคับข้อใหม่ (ไม่คืนข้อที่รัวตอบผิดไปแล้ว)
  if v_p.cooldown_until is not null then
    if v_p.cooldown_until > now() then
      return jsonb_build_object('cooldown', true, 'cooldown_until', v_p.cooldown_until);
    end if;
    update public.boss_raid_participants
      set cooldown_until = null, wrong_streak_current = 0,
          current_question_id = null, question_started_at = null
      where id = p_participant_id;
    v_p.current_question_id := null;
    v_p.question_started_at := null;
  end if;
...
begin
  ...
  -- Item 4b: ต้องแม่นยำ >= 40% (ตอบถูก / ตอบทั้งหมดของรอบนี้) ถึงมีสิทธิ์รับไข่.
  -- กรองก่อน แล้วค่อยจัดอันดับด้วย total_damage ตามเดิม.
  for v_row in
    with elig as (
      select p.id as participant_id, p.user_id, p.total_damage, p.joined_at
      from public.boss_raid_participants p
      where p.session_id = p_session_id
        and p.total_damage > 0
        and coalesce(
          (select count(*) filter (where a.is_correct)::numeric / nullif(count(*), 0)
           from public.boss_raid_answers a where a.participant_id = p.id), 0
        ) >= 0.40
    )
    select participant_id, user_id, total_damage,
           row_number() over (order by total_damage desc, joined_at asc) as rnk
    from elig
