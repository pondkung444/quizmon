...
do $$
begin
  if to_regprocedure('public.question_factory_create_run(uuid,text,text,text,text,text,text,text,jsonb,text,text,text,text,jsonb,integer,integer,integer,integer,smallint,smallint,jsonb)') is not null then
    raise exception 'question_factory_create_run already exists';
  end if;
end
$$;
...
create or replace function public.question_factory_create_run(
  ...
  p_resolved_profile jsonb,
  p_blueprint_id text,
  p_blueprint_version text,
  p_blueprint_schema_version text,
  p_blueprint_checksum text,
  p_resolved_blueprint jsonb,
  p_target_active integer,
  p_preferred_batch_size integer,
  p_max_batch_size integer,
  p_max_generated_items integer,
  p_max_revisions_per_slot smallint,
  p_max_technical_retries smallint,
  p_slots jsonb
)
returns jsonb
language plpgsql
...
begin
  ...
  if p_resolved_profile is null or jsonb_typeof(p_resolved_profile) <> 'object' then
    raise exception 'resolved_profile must be an object';
  end if;
  if p_resolved_blueprint is null or jsonb_typeof(p_resolved_blueprint) <> 'object' then
    raise exception 'resolved_blueprint must be an object';
  end if;
  ...
  if jsonb_array_length(p_slots) <> p_target_active then
    raise exception 'slot count must equal target_active';
  end if;
  if exists (
    select 1 from jsonb_array_elements(p_slots) item
    where jsonb_typeof(item) <> 'object'
       or nullif(btrim(item->>'slot_key'), '') is null
       or (item->>'ordinal') is null
       or (item->>'ordinal') !~ '^[1-9][0-9]*$'
  ...
  if (select count(*) from jsonb_array_elements(p_slots)) <>
     (select count(distinct item->>'slot_key') from jsonb_array_elements(p_slots) item) then
    raise exception 'slot_key values must be unique';
  end if;
  if (select count(*) from jsonb_array_elements(p_slots)) <>
     (select count(distinct (item->>'ordinal')::integer) from jsonb_array_elements(p_slots) item) then
    raise exception 'slot ordinals must be unique';
  end if;
  ...
  if found then
    ...
    where run_id = v_existing.id and event_type = 'RUN_CREATED'
  ...
  values
    (p_profile_id, p_profile_version, p_profile_schema_version, p_profile_checksum, p_resolved_profile)
  on conflict (profile_id, profile_version, checksum) do nothing;

  select id into strict v_profile_snapshot_id
  from public.question_factory_profile_snapshots
  ...
  values
    (p_blueprint_id, p_blueprint_version, p_blueprint_schema_version, v_profile_snapshot_id,
  ...
  where blueprint_id = p_blueprint_id and blueprint_version = p_blueprint_version
    and checksum = p_blueprint_checksum;
  if v_slot_id <> v_profile_snapshot_id then
    raise exception 'blueprint identity is already pinned to a different profile snapshot';
  end if;

  insert into public.question_factory_runs (
    run_key, scope_key, profile_snapshot_id, blueprint_snapshot_id, target_active,
    preferred_batch_size, max_batch_size, max_generated_items,
    max_revisions_per_slot, max_technical_retries, created_by
  ) values (
    p_run_key, p_scope_key, v_profile_snapshot_id, v_blueprint_snapshot_id, p_target_active,
    p_preferred_batch_size, p_max_batch_size, p_max_generated_items,
    p_max_revisions_per_slot, p_max_technical_retries, p_created_by
  ...
  values
    (v_run_id, 'RUN_CREATED', null, 'created', 'RUN_INITIALIZED',
     jsonb_build_object('target_active', p_target_active, 'slot_count', jsonb_array_length(p_slots),
...
revoke all on function public.question_factory_create_run(
...
grant execute on function public.question_factory_create_run(
  uuid, text, text, text, text, text, text, text, jsonb, text, text, text, text, jsonb,
  integer, integer, integer, integer, smallint, smallint, jsonb
) to service_role;
...
comment on function public.question_factory_create_run(
  uuid, text, text, text, text, text, text, text, jsonb, text, text, text, text, jsonb,
  integer, integer, integer, integer, smallint, smallint, jsonb
) is 'Atomically creates or idempotently replays a Question Factory run with immutable snapshots, planned slots, and factual initialization events. Service role only.';
