-- Boss Raid balance tune (2026-09-07): EXPERIMENTAL, not final.
-- Monte Carlo sim on real pet stat data says rooms currently die a touch too fast.
-- Two coordinated constant bumps only (no schema / no other constants / no UI):
--   1) start_boss_raid_game.c_target_correct 125 -> 200  (boss ~60% tankier)
--   2) submit_boss_raid_answer.v_crystal_damage  rank*6 -> rank*10  (crystal chips faster)
-- ปอนด์ runs 3 live test rooms today (panuwat.pond@gmail.com / daou@mail.com) and may
-- ask for another pass — keep this diff minimal / easy to re-tune.
...
begin
  ...
  if v_is_correct and v_damage > 0 then
    ...
    v_crystal_damage := v_new_rank * 10;
...
declare
  ...
  c_target_correct constant numeric := 200;  -- balance fix 2026-09-07: บอสอึดขึ้น ~60% (125->200) คู่กับ crystal_damage mult 6->10, ยัง tuning ต่อ
