-- Migration: 20260903150000_boss_raid_phase_2_enrage
-- Classroom Boss Raid — Phase 2: Event "บอสโกรธ" (Boss Enraged)
-- อ้างอิง: task brief boss-raid-enrage (เคาะกับปอนด์ครบ 2026-09-03)
--
-- เวอร์ชัน deterministic ของ weak_point — trigger ด้วย HP milestone แทน % roll:
--   * boss_hp ตกผ่าน 75% / 50% / 25% ของ boss_hp_max ครั้งแรก (edge, 1 ครั้ง/milestone/เกม)
--   * effect = บัฟดาเมจทั้งห้อง ×2.5 นาน 15 วิ (passive window เหมือน weak_point, ไม่มีคำถามพิเศษ)
--   * gate เดียวกัน — ไม่ trigger ถ้ามี event อื่น active. ถ้า milestone ข้ามตอนมี event อื่นอยู่
--     -> mark fired เฉยๆ ไม่ queue (ข้ามไปเลย)
--
-- refactor: weak_point + enrage อ่าน multiplier จาก active_event->>'multiplier' ตัวเดียว
--   (weak_point roll ตอนนี้เขียน 'multiplier':2 ด้วย) — เลิก hardcode ×2 แยก
--
-- Base = submit_boss_raid_answer จาก 20260903140000_boss_raid_phase_2_chosen_warrior.sql
...
alter table public.boss_raid_sessions
  add column if not exists enrage_milestones_fired jsonb not null default '[]'::jsonb;

comment on column public.boss_raid_sessions.enrage_milestones_fired is
  'milestone HP% ที่ event "บอสโกรธ" fire ไปแล้ว เช่น [75,50] — edge-trigger 1 ครั้ง/milestone/เกม.';
...
alter table public.boss_raid_event_log
  ...
  check (event_type in ('weak_point', 'meteor', 'meteor_attempt',
                        'chosen_warrior', 'chosen_warrior_answer', 'enrage'));

-- ===== 2. submit_boss_raid_answer — multiplier จาก jsonb + enrage milestone trigger =====
...
begin
  ...
  -- บัฟดาเมจ active (weak_point / enrage) — อ่าน multiplier จาก jsonb ตัวเดียว
  ...
  -- active_event ยัง "ว่าง" ไหม (ตอนเริ่ม call — ยังไม่มีอะไรถูกเซ็ตใน call นี้จนถึงจุดนี้)
    ...
    if jsonb_array_length(v_new_milestones) > 0 then
      if v_event_free then
        ...
        if found then
          ...
          -- race: มี event อื่นแทรกพอดี — mark fired เฉยๆ
          update public.boss_raid_sessions
            set enrage_milestones_fired = enrage_milestones_fired || v_new_milestones
            where id = v_p.session_id;
        end if;
      else
        -- milestone ข้ามตอนมี event อื่น active — mark fired ไม่ queue
        update public.boss_raid_sessions
          set enrage_milestones_fired = enrage_milestones_fired || v_new_milestones
          where id = v_p.session_id;
      end if;
...
-- ============================================================
-- Rollback:
--   re-apply 20260903140000_boss_raid_phase_2_chosen_warrior.sql (submit_boss_raid_answer)
...
--   alter table boss_raid_sessions drop column enrage_milestones_fired;
-- ============================================================
